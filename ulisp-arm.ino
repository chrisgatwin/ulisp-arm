...
/* fix issues with memory type conficts for teensy 4 */
...
#define BUFFERSIZE 34  // Number of bits+2
  ...
  uint8_t _end;
    ...
    #define WORKSPACESIZE 20480             /* Cells (8*bytes) */
    #define SYMBOLTABLESIZE 512             /* Bytes */
    ...
    #define WORKSPACESIZE 40480             /* Cells (8*bytes) */
    #define SYMBOLTABLESIZE 512             /* Bytes */
    #define SDCARD_SS_PIN 10
  #endif
  extern uint8_t _end;
...
#elif defined(ARDUINO_SAM_DUE) || (defined(TEENSYDUINO) && defined(__MK20DX256__))
inline int serial1read () { while (!Serial1.available()) testescape(); return Serial1.read(); }
...
#elif defined(TEENSYDUINO) && (defined(__MK64FX512__) || defined(__MK66FX1M0__) || defined(__IMXRT1062__))
inline int serial1read () { while (!Serial1.available()) testescape(); return Serial1.read(); }
inline int serial2read () { while (!Serial2.available()) testescape(); return Serial2.read(); }
inline int serial3read () { while (!Serial3.available()) testescape(); return Serial3.read(); }
inline int serial4read () { while (!Serial4.available()) testescape(); return Serial4.read(); }
inline int serial5read () { while (!Serial5.available()) testescape(); return Serial5.read(); }
#endif
#if defined(sdcardsupport)
...
void serialend (int address) {
  ...
  else if (address == 4) {Serial2.flush(); Serial2.end(); }
  else if (address == 5) {Serial3.flush(); Serial3.end(); }
  #endif
}
...
void checkanalogread (int pin) {
  ...
  if (!(pin>=14 && pin<=23)) error(ANALOGREAD, PSTR("invalid pin"), number(pin));
#elif defined(TEENSYDUINO) && (defined(__MK64FX512__) || defined(__MK66FX1M0__))
  if (!((pin>=14 && pin<=23) || (pin>=31 && pin<=41))) error(ANALOGREAD, PSTR("invalid pin"), number(pin));
#elif defined(TEENSYDUINO) && defined(__IMXRT1062__)
  if (!(pin>=14 && pin<=27)) error(ANALOGREAD, PSTR("invalid pin"), number(pin));
#endif
}
...
#elif defined(TEENSYDUINO) && defined(__MK20DX256__)
  if (!((pin>=3 && pin<=6) || (pin>=9 && pin<=10) || (pin>=20 && pin<=23) ))
      error(ANALOGWRITE, PSTR("invalid pin"), number(pin));
#elif defined(TEENSYDUINO) && defined(__MK64FX512__)
  if (!((pin>=2 && pin<=10) || pin==14 || (pin>=20 && pin<=23) || (pin>=29 && pin<=30) || (pin>=35 && pin<=38)))
      error(ANALOGWRITE, PSTR("invalid pin"), number(pin));
#elif defined(TEENSYDUINO) && defined(__MK66FX1M0__)
  if (!((pin>=2 && pin<=10) || pin==14 || (pin>=16 && pin<=17) || (pin>=20 && pin<=23) || (pin>=29 && pin<=30) || (pin>=35 && pin<=38)))
      error(ANALOGWRITE, PSTR("invalid pin"), number(pin));
#elif defined(TEENSYDUINO) && defined(__IMXRT1062__)
  if (!((pin>=0 && pin<=15) || (pin>=18 && pin<=19) || (pin>=22 && pin<=25) || (pin>=28 && pin<=29) || (pin>=33 && pin<=39)))
      error(ANALOGWRITE, PSTR("invalid pin"), number(pin));
#endif
}
